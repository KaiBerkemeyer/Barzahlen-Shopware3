...
class Shopware_Controllers_Frontend_PaymentBarzahlen extends Shopware_Controllers_Frontend_Payment {
  ...
  public function gatewayAction(){
    ...
    if (!$userinfo) {
      Shopware()->Session()->BarzahlenPaymentError = "Es gab einen Fehler bei der Daten&uuml;bertragung. Bitte versuchen Sie es erneut oder w&auml;hlen Sie eine andere Zahlungsmethode.";
  ...
  public function notifyAction(){
    ...
    $notify = new Barzahlen_Notification($shopId, $notificationKey, $_GET);
    ...
    if($notify->isValid()) {
      ...
      $orderId = $this->_getOrderId($_GET['transaction_id']);
      $orderNumber = isset($_GET['order_id']) ? $_GET['order_id'] : $this->_getOrderNumber($orderId);
      ...
      if(!$this->_checkInvoiceData($orderId)) {
        $order->setPaymentStatus($orderId, self::PAYMENT_VERIFY);
        $this->_logError("Payment for order ".$orderNumber." couldn't be updated by notification. Invoice data invalid.");
        return;
      }
      ...
      switch($_GET['state']) {
        case 'paid':
          ...
          $this->_setClearedDate($_GET['transaction_id']);
          break;
        case 'expired':
  ...
  /**
   * Checks that the invoice data match with the saved ones.
   *
   * @param integer $orderId internal order id
   * @return boolean
   */
  protected function _checkInvoiceData($orderId) {

    $sql = 'SELECT count(*) as count FROM s_order WHERE id=? AND invoice_amount=? AND currency=?';
    $count = Shopware()->Db()->fetchOne($sql, array($orderId, $_GET['amount'], $_GET['currency']));
